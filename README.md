...
当前版本为 **v0.3.0（构建号 1）**，最低支持 iOS 17，仅支持 iPhone。
...
需要 macOS、Xcode 16.3+、Swift 6.1+ 和 iOS 17+ Simulator。v0.3 最终验证使用 macOS 26.6.2、Xcode 26.6、Swift 6.3.3 和 iOS 26.5 Simulator。SwiftPM 锁定：
...
`.oboe-backup` v3 是未压缩、未加密的 UTF-8 NDJSON，使用固定记录顺序和 SHA-256 footer。它包含学习数据、设置及以下 v0.3 收集箱记录：
...
备份不包含 API Key、AI 连接配置、内置只读词库、搜索派生索引、本机快照、图片附件文件或尚未导入的共享队列文件。跨设备恢复后，收集箱正文会保留，缺失的图片引用会安全降级。

恢复前 App 会检查格式版本、UTF-8/LF、记录顺序、数量、SHA-256、外键、调度状态和算法版本；预检不修改当前数据库。正式恢复是**完整替换**而不是合并导入，替换前会创建回滚快照，失败或进程中断时恢复到已验证的数据库。Oboe 可恢复 v1、v2、v3 备份，并明确拒绝未来格式和未知调度算法。
